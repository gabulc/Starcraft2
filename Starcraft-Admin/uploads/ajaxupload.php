<?php
    function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
        ...
        if($filesize > 0){	
            ...
            //print_r($filename);
            if($filesize < 1){ 
                    $errorList[] = "El tamaño del achivo está vacío";
            }
            if($filesize > $maxlimit){ 
                    $errorList[] = "El tamaño del archivo es muy grande";
            }
                        ...
                        if(!file_exists($save)){
                            ...
                            if($maxH == null){
                                ...
                                switch($filetype){
                                    ...
                                    break;
                                    case "bmp":
                                        $image = @imagecreatefromxbm($_FILES[$fileName]['tmp_name']);
                                    break;
                                }
                                ...
                                switch($filetype){
                                    ...
                                    case "png":
                                        ...
                                        break;
                                    case "bmp":
                                        if(!@imagepng($image_p, $save, 100)){
                                            $errorList[]= "Permiso denegado [BMP]";
                                        }
                                        break;
                                }
                                @imagedestroy($filename);
                            }else{
                                $errorList[]= "No puede carga la imagen, el archivo ya existe";
                            }	
                    }
                }		
            }
        }else{
            $errorList[]= "No seleccionado algún archivo";
        }
        if(!$match){
            $errorList[]= "El tipo de archivo no está permitido: $filename";
        }
...
<?php
    if($imgUploaded){
            echo '<img src="http://localhost/Starcraft-Admin/Imagenes/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Cargo satisfactoriamente!<br /><img src="'.$upload_image.'" border="0" />';
    }else{
            echo '<img src="http://localhost/Starcraft-Admin/Imagenes/error.gif" width="16" height="16px" border="0" style="marin-bottom: -3px;" /> Error encontrado: ';
